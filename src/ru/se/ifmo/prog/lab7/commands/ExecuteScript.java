...
public class ExecuteScript extends Command {
	...
	public Response execute(String[] args, Integer stacksize, Dragon dragon, CommandManager commandmanager, CollectionData collectiondata, DatabaseConnector connector, String login, String password) {
		...
		for (CommandShallow command : commandsList) {
			response.addLines(command.getCommand().execute(command.getArguments(), ++stacksize, command.getDragon(), commandmanager, collectiondata, connector, login, password).getMessage());
		}
		return response;
	}
}
